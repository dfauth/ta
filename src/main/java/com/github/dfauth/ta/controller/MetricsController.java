...
public class MetricsController {
    ...
    public TradingMetrics metrics(@RequestParam("theme") Optional<Theme> theme) {
        return metrics(ALL, theme);
    }

    @GetMapping("/metrics/mode/{mode}")
    @ResponseStatus(HttpStatus.OK)
    public TradingMetrics metrics(@PathVariable Mode mode, @RequestParam("theme") Optional<Theme> theme) {
        Predicate<Position> p = theme.map(t -> t.and(mode)).orElse(mode);
