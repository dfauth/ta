...
public class Optionals {
    ...
    public static <T> Optional<T> eitherOrBoth(Optional<T> t1, Optional<T> t2, BinaryOperator<T> f2) {
        return t1.map(_t1 -> t2.map(_t2 -> f2.apply(_t1,_t2)).orElse(_t1)).or(() -> t2);
    }
    ...
    public static <T> Optional<T> allPresent(Function<T,?> fn, List<Optional<T>> ts) {
        return headAndTail(ts).map((h,t) -> {
            if(t.isEmpty()) {
                ...
                return h.flatMap(_h -> allPresent((Function<T,?>)fn.apply(_h), t));
            }
        });
    }
}
