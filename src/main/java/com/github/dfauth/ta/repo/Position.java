...
public class Position implements com.github.dfauth.ta.model.Position {
    ...
    @JsonIgnore
    @Column(name = "weightedHoldingTime")
    private long unitsHoldingDays;
    ...
    public LocalDate getLast() {
        return getLastTimestamp().toLocalDateTime().toLocalDate();
    }

    public Optional<BigDecimal> getDividends() {
        return Optional.ofNullable(payments).flatMap(p -> p.stream().filter(payment -> payment.getTxnType().isDividend()).map(Payment::getValue).reduce(BigDecimal::add));
    }
    ...
    private Position apply(List<Trade> t) {
        ...
        t.stream().filter(_t -> this.last != null).forEach(_t -> this.unitsHoldingDays =+ calculateWeightedHoldingTime(this.date.toInstant(), this.last.toInstant(), Optional.ofNullable(getSize()).orElse(0)));
    ...
    public Optional<Double> getCagr() {
        Optional<Double> p = periods.apply(getUnitsHoldingDays(), getUnitsPurchased());
    ...
    public BigDecimal getWeightedHoldingTime() {
        return BigDecimal.valueOf((unitsHoldingDays / unitsPurchased) / 365.25).setScale(2, RoundingMode.HALF_UP);
    }
    ...
    public Position merge(Position other) {
        return new Position(
            ...
            last.toLocalDateTime().toLocalDate().isAfter(other.getLast()) ? last : other.last,
            unitsPurchased + other.unitsPurchased,
            unitsSold + other.unitsSold,
            unitsHoldingDays + other.unitsHoldingDays,
