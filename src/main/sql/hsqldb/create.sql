DROP TABLE CODES IF EXISTS;
CREATE TABLE IF NOT EXISTS CODES (
    CODE VARCHAR(255) NOT NULL,
    PRIMARY KEY (CODE)
);
DROP TABLE PRICE IF EXISTS
CREATE TABLE IF NOT EXISTS PRICE (
    CODE VARCHAR(255) NOT NULL,
    DATE DATETIME NOT NULL,
    OPEN DECIMAL(8,3),
    HIGH DECIMAL(8,3),
    LOW DECIMAL(8,3),
    CLOSE DECIMAL(8,3),
    VOLUME INTEGER,
    PRIMARY KEY (CODE, DATE)
);
DROP TABLE VALUATION IF EXISTS
CREATE TABLE IF NOT EXISTS VALUATION (
    CODE VARCHAR(255) NOT NULL,
    DATE DATETIME NOT NULL,
    RATING INTEGER,
    BUY INTEGER,
    HOLD INTEGER,
    SELL INTEGER,
    TARGET DECIMAL(8,3),
    PRIMARY KEY (CODE, DATE)
);
DROP TABLE INDX IF EXISTS
CREATE TABLE IF NOT EXISTS INDX (
    IDX VARCHAR(255) NOT NULL,
    DATE DATETIME NOT NULL,
    CODE VARCHAR(255) NOT NULL,
    PRIMARY KEY (IDX,DATE,CODE)
);
DROP TABLE TRADE IF EXISTS
CREATE TABLE IF NOT EXISTS TRADE (
    ID INTEGER NOT NULL IDENTITY,
    CONFIRMATION_NO VARCHAR(255) NOT NULL,
    DATE DATETIME NOT NULL,
    CODE VARCHAR(255) NOT NULL,
    SIZE INTEGER,
    PRICE DECIMAL(8,3),
    COST DECIMAL(8,2),
    SIDE INT,
    NOTES VARCHAR(100000),
    THEME INTEGER,
    PRIMARY KEY(ID),
    UNIQUE (CONFIRMATION_NO)
    -- FOREIGN KEY (CODE) REFERENCES CODES(CODE)
);
DROP TABLE RANKING IF EXISTS
CREATE TABLE IF NOT EXISTS RANKING (
    ID INTEGER NOT NULL,
    DATE DATETIME NOT NULL,
    CODE VARCHAR(255) NOT NULL,
    RANK INTEGER,
    PRIMARY KEY (ID,DATE,CODE)
);
DROP TABLE PAYMENT IF EXISTS
CREATE TABLE IF NOT EXISTS PAYMENT (
    ID INTEGER NOT NULL IDENTITY,
    TYPE INTEGER NOT NULL,
    DATE DATETIME NOT NULL,
    DETAIL VARCHAR(255) NOT NULL,
    VALUE DECIMAL(8,2) NOT NULL,
    BALANCE DECIMAL(12,2) NOT NULL,
    SIDE INTEGER,
    CODE VARCHAR(255),
    CONTRACTNO VARCHAR(255),
    EX_DIVIDEND_DATE DATE,
    PRIMARY KEY (ID),
    FOREIGN KEY (CODE) REFERENCES CODES(CODE)
);
DROP TABLE POSITION IF EXISTS;
CREATE TABLE IF NOT EXISTS POSITION (
    DATE DATETIME NOT NULL,
    CODE VARCHAR(255) NOT NULL,
    LAST DATETIME NOT NULL,
    UNITS_PURCHASED INTEGER,
    UNITS_SOLD INTEGER,
    WEIGHTED_HOLDING_TIME INTEGER,
    PURCHASE_VALUE DECIMAL(12,3),
    SALE_VALUE DECIMAL(12,3),
    COMMISSION DECIMAL(12,3),
    PRIMARY KEY(DATE,CODE)
);
DROP TABLE POSITION_TRADES IF EXISTS;
CREATE TABLE IF NOT EXISTS POSITION_TRADES (
    POSITION_DATE DATETIME NOT NULL,
    POSITION_CODE VARCHAR(255) NOT NULL,
    TRADES_ID INTEGER,
    FOREIGN KEY (TRADES_ID) REFERENCES TRADE(ID),
    FOREIGN KEY (POSITION_DATE,POSITION_CODE) REFERENCES POSITION(DATE,CODE),
);
DROP TABLE POSITION_PAYMENTS IF EXISTS;
CREATE TABLE IF NOT EXISTS POSITION_PAYMENTS (
    POSITION_DATE DATETIME NOT NULL,
    POSITION_CODE VARCHAR(255) NOT NULL,
    PAYMENTS_ID INTEGER,
    FOREIGN KEY (PAYMENTS_ID) REFERENCES PAYMENT(ID),
    FOREIGN KEY (POSITION_DATE,POSITION_CODE) REFERENCES POSITION(DATE,CODE),
);
DROP TABLE MKTDEPTH IF EXISTS;
CREATE TABLE IF NOT EXISTS MKTDEPTH (
    DATE DATETIME NOT NULL,
    CODE VARCHAR(255) NOT NULL,
    BUYERS INTEGER,
    BUYER_SHARES INTEGER,
    SELLERS INTEGER,
    SELLER_SHARES INTEGER,
    PRICE DECIMAL(12,3),
    CHANGE DECIMAL(12,3),
    VOLUME INTEGER,
    PRIMARY KEY(DATE,CODE)
);
