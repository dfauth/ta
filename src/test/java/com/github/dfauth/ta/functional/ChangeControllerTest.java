...
public class ChangeControllerTest {
    ...
    public void testIt() {
        ...
        BigDecimal sma = result1.stream().collect(ringBufferCollector(new Integer[3],Lists.<Integer,BigDecimal>mapList(BigDecimal::valueOf).andThen(SMA)));
        assertEquals(4.66667, sma.doubleValue(), 0.001d);
    }

    @Test
    public void testItAgain() {
        BigDecimal sma = INPUT.stream().collect(adjacent(increment)).stream().collect(ringBufferCollector(new Integer[3],Lists.<Integer,BigDecimal>mapList(BigDecimal::valueOf).andThen(SMA)));
        assertEquals(4.66667, sma.doubleValue(), 0.001d);
    }
}