...
public class RSITest {
    ...
    public void testWGX() {
        int period = 14+1;
        Tuple2<List<Dated<BigDecimal>>, List<Dated<BigDecimal>>> subList = splitAt(mapList(mapList(TestData.WGX, o -> o), Dated.apply(PriceAction::getClose)), TestData.WGX.size() - period);
        assertEquals(
                37.03d,
                calculateRSI(subList._2().stream(),period).map(Dated::getPayload).map(BigDecimal::doubleValue).get(),
                0.01d
        );
    }
